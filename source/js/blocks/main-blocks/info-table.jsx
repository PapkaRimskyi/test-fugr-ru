...
export default function InfoTable(props) {
  const { receivedData, userCellDelegationHandler, typeSortHandler } = props;
  const [sortDirection, sortTypeButton] = props.sortInfo;
  const sortTypes = ['id', 'firstName', 'lastName', 'email', 'phone'];
  return (
    <table className="info-table" onClick={userCellDelegationHandler}>
      <tbody>
        <tr className="info-table__column-types" onClick={typeSortHandler}>
          {sortTypes.map((type) => (
            <th className={`info-table__column-type ${sortTypeButton === type && sortDirection ? 'info-table__column-type--sort-up' : 'info-table__column-type--sort-down'}`} id={`type-${type}`} key={type} tabIndex="0" aria-label={`Сортировка по ${type}`}>
              {type}
        ...
        {receivedData.map((info) => (
          <tr className="info-table__user" key={info.email} tabIndex="0">
            <td>{info.id}</td>
